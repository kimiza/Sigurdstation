...
/obj/item/weapon/reagent_containers/food/condiment
	...
	attack(mob/M as mob, mob/user as mob, def_zone)
		...
		if(M == user)
			...
			msg_admin_attack("ATTACK: [user.name] ([user.ckey]) fed [M.name] ([M.ckey]) with [src.name] (INTENT: [uppertext(user.a_intent)]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)")
